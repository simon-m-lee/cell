...
import 'ai_tissue_command_domain_stub.dart'
    if (dart.library.io) 'ai_tissue_command_domain_io.dart';
export 'ai_tissue_command_domain_stub.dart'
    if (dart.library.io) 'ai_tissue_command_domain_io.dart'
    show HttpInterpreter;
...
/// deliberately portable — it imports only `dart:async` and
/// `dart:convert`, and does not depend on `cell`,
...
/// the same values `platformEnvironment` would, and either can
...
/// `platformEnvironment` and it does not merge with
...
/// ```dart
/// final merged = {
///   if (platformEnvironment['AI_ENDPOINT'] != null)
///     'endpoint': platformEnvironment['AI_ENDPOINT'],
///   if (platformEnvironment['AI_API_KEY'] != null)
///     'apiKey': platformEnvironment['AI_API_KEY'],
///   if (platformEnvironment['AI_MODEL'] != null)
///     'model': platformEnvironment['AI_MODEL'],
///   ...fileConfig,
/// };
...
final class AiConfig {
  ...
  static AiConfig fromJsonFile(String path) {
    final raw = readJsonFileSync(path);
  ...
  /// config. It does not consult `platformEnvironment` at runtime
...
Future<void> main(List<String> args) async {
  ...
  if (!live) {
    ...
    final endpoint = platformEnvironment['AI_ENDPOINT'];
    final apiKey = platformEnvironment['AI_API_KEY'];
    final model = platformEnvironment['AI_MODEL'] ?? 'gpt-4o-mini';
