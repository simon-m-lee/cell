...
void main() {
  ...
  group('Filter extra', () {
    test('onError is optional when predicate throws', () async {
      final b = bind(Filter<int>((n) => throw StateError('p')));
      addTearDown(b.probe.stop);
      await b.gate.emitAsync(1);
      await b.probe.settle();
      expect(b.probe.payloads, isEmpty);
    ...
    test('empty source is silent', () async {
      final b = bind(Filter<int>((n) => true));
      addTearDown(b.probe.stop);
      await b.probe.settle();
      expect(b.probe.payloads, isEmpty);
  ...
  group('FilterNotNull extra', () {
    test('all-null source is silent', () async {
      final b = bind(FilterNotNull<int?>());
      addTearDown(b.probe.stop);
      await b.gate.emitAsync(null);
      await b.probe.settle();
      expect(b.probe.payloads, isEmpty);
  ...
  group('AsyncFilter extra', () {
    test('false predicate drops the pulse', () async {
      final b = bind(AsyncFilter<int>((n) async => false));
      addTearDown(b.probe.stop);
      await b.gate.emitAsync(1);
      await b.probe.settle();
      expect(b.probe.payloads, isEmpty);
  ...
  group('AsyncFilterWithTimeout extra', () {
    test('wrong types call onError', () async {
      final errors = <Object>[];
      final IngressHandle<Object> gate = Cell.ingress<Object>();
      final out = AsyncFilterWithTimeout<int>(
        (n) async => true,
  ...
  group('performance extra', () {
    test('Filter pass-through handles 200 ints', () async {
      final b = bind(Filter<int>((n) => true));
      addTearDown(b.probe.stop);
      for (var i = 0; i < 200; i++) {
