...
class AsyncSynapses<P extends Pulse, L extends Cell> extends SynapsesBase<P,L> {
  ...
  @override
  Function? get _relay => _source._relay;

  @override
  QueueList<PulseBase>? get _buffer => _source._buffer;

  @override
  Box<Timer>? get _timerBox => _source._timerBox;

  @override
  FilterRule<P>? get _filter => _source._filter as FilterRule<P>?;

  @override
  PropagationPolicy? get _policy => _source._policy;
  ...
  Future<void> _broadcast(PulseBase pulse) async {
    ...
    final relay = _relay;
    if (relay != null) {
      if (_source.every((c) => pulse._checker.contains(c))) {
        pulse._complete();
        return;
      }
      relay.call(pulse);
  ...
  Future<void> _flushBatch() async {
    ...
    if (buffered.isNotEmpty) {
      final batched = Pulse.batch(List<Pulse>.from(buffered));
  ...
  FutureOr<void> _debounced(PulseBase p) async {
    ...
    final window = _policy!.debounceTime;
    if (window == Duration.zero) {
      timerBox.value = null;
      await _broadcast(p);
      return;
    }
    timerBox.value = Timer(window, () async {
      final buffer = _buffer;
      if (buffer != null) {
  ...
  Future<void> _throttled(PulseBase p) async {
    final timerBox = _timerBox;
    if (timerBox == null) return;
    if (timerBox.value != null) return;

    await _broadcast(p);
    final window = _policy!.throttleTime;
    if (window == Duration.zero) return;
    timerBox.value = Timer(window, () {
      timerBox.value?.cancel();
      timerBox.value = null;
    });
  }
  ...
  Future<void> _resilient(PulseBase p) async {
    ...
    try {
      ...
      // by setting a timerBox.state for a cooldown period.
...
abstract class SynapsesBase<P extends Pulse, C extends Cell> extends IterableBase<C> implements Synapses<P,C> {
  ...
  void _flushBatch() {
    ...
    final snapshot = List<Pulse>.from(buffer);
    buffer.clear();
    _broadcast(Pulse.batch(snapshot) as PulseBase);
  }

  /// Cancels a [PropagationStrategy.sample] heartbeat when nobody is listening.
  ///
  /// [Timer.periodic] is otherwise never cancelled, which would leak the
  /// timer for the rest of the isolate lifetime.
  void _stopSampleHeartbeat() {
    if (_policy?.strategy != PropagationStrategy.sample) return;
    final timerBox = _timerBox;
    timerBox?.value?.cancel();
    timerBox?.value = null;
  }
  ...
  void _buffered(PulseBase p) {
    ...
    if (buffer.length == 1 && timerBox != null && timerBox.value == null) {
      final window = _policy!.throttleTime;
      if (window == Duration.zero) {
        _flushBatch();
        return;
...
enum PropagationStrategy {
  ...
  /// *   **Non‑obvious**: The heartbeat rebroadcasts the *first* pulse
  ///     captured when the timer starts. The pulse cycle checker then
  ///     drops later ticks of that same instance, so observers typically
  ///     see one delivery. Unlinking the last observer cancels the
  ///     [Timer.periodic] so it does not leak.
  sample,
...
class PropagationPolicy {
  ...
  bool operator ==(Object other) {
    if (identical(this, other)) return true;
    return other is PropagationPolicy && other._record == _record;
  }
