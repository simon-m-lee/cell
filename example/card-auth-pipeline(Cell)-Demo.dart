...
// ─────────────────────────────────────────────────────────────────────────────
// CARD AUTH PIPELINE (Cell) — one FlowInstructionChain wraps the gate
//
// Sibling: card-auth-pipeline(tissue)-Demo.dart (two gates + observers)
// Walkthrough: card-auth-pipeline(Cell)-WalkThrough.md
//
// THE CHANGE FROM THE CardAuthInstruction VERSION
//   The previous build of this demo wrapped the whole gate inside one custom
//   FlowInstructionBase subclass (CardAuthInstruction). This build shows the
//   other idiomatic cell_flow path: chain STOCK Rx operators with `+` into a
//   single FlowInstructionChain, then materialize the chain once.
//
//   cardAuth = MapValue<AuthAttempt, CardAuthRecord>(snapshot)     // Rx map
//            + DistinctUntilChanged<CardAuthRecord>(by decision)   // Rx distinctUntilChanged
//            + Filter<CardAuthRecord>(decline || stepUp)           // Rx filter
//            + Tap<CardAuthRecord>(write the books)                // Rx tap / do
//            + MapValue<CardAuthRecord, Decision>(emit decision);  // Rx map
//
//   cardAuth.toHandle(source: attemptIn.cell)   // exactly once
//
//   Each `+` returns a FlowInstruction, so the whole expression is ONE
//   FlowInstructionChain. The instruction is still just a blueprint; the
//   single `toHandle` call compiles it into the live authCell.
//
// DOCUMENTED DEVIATION FOR STOCK OPERATORS
//   DistinctUntilChanged keeps its own internal "previous value" and cannot
//   be reset from outside. The class-based variant used a hand-rolled latch
//   that ACK could zero. Here ACK still appends its ledger row, but it does
//   NOT reset the chain's DistinctUntilChanged.
//
//   The scenario sequence is designed so this does not change the visible
//   output: every repeated DECLINE is separated by an approve (3a / 6 / 9a),
//   so the stock distinct operator lets the next DECLINE through anyway.
//   This is the classic trade-off: stock operators = less code, but no
//   external reset of operator state.
//
// THE SEAM — Flow owns the decision, Tissue owns the books.
//
// Two locks, two owners, two talk-track sentences:
//   1. Receptor lock on authCell — the chained operators run riskOf,
//      DistinctUntilChanged, Filter, and the DECLINE / STEP-UP book writes
//      (the Tap side effect). Returns a Decision. Never calls placeHold /
//      capture / voidHold.
//   2. Tissue lock on each collection — ledger.add, issuerQ.addLast,
//      available / held / holdsMap writes from money methods.
//
...
//    • To keep the demo's console output deterministic, the trace prints
//      are emitted directly by the writers: the Tap side effect, the ACK
//      observer, `placeHold`, `capture`, `voidHold`, and the mccBlock add
//      in scenario 2.
//
...
//
// 6. ACK does not reset the stock DistinctUntilChanged:
//    • DistinctUntilChanged has no public reset API. The ACK observer
//      therefore appends its ACK row only. The scenario sequence keeps
//      the same visible output because every repeated DECLINE is
//      separated by an approve.
//
// ─────────────────────────────────────────────────────────────────────────────
...
//   │                                ▼                                        │
//   │   ┌────────────────────────────────────────────────────────────────┐    │
//   │   │  ONE FlowInstructionChain (stock Rx operators, `+` chained)    │    │
//   │   │                                                                │    │
//   │   │  MapValue:  AuthAttempt → (attempt, decision)   [Rx map]       │    │
//   │   │  DistinctUntilChanged: drop repeat decision     [Rx distinct]  │    │
//   │   │  Filter:   keep decline / stepUp                [Rx filter]    │    │
//   │   │  Tap:      ledger + issuerQ book writes         [Rx tap/do]    │    │
//   │   │  MapValue: (attempt, decision) → Decision       [Rx map]       │    │
//   │   │                                                                │    │
//   │   │  Materialized once:  toHandle(source: attemptIn.cell)          │    │
//   │   │  Live gate:          authHandle.cell  →  emits Pulse<Decision> │    │
//   │   └────────────────────────────────────────────────────────────────┘    │
//   │                                                                         │
//   └─────────────────────────────────────────────────────────────────────────┘
//                                       │
...
//
//   ╔═══════════════════════════════════════════════════════════════════════╗
//   ║  card-auth-pipeline(Cell)-Demo.dart                                    ║
//   ║  One FlowInstructionChain:                                             ║
//   ║  MapValue → DistinctUntilChanged → Filter → Tap → MapValue             ║
//   ║  Flow decides. Tissue keeps the books.                                 ║
//   ╚═══════════════════════════════════════════════════════════════════════╝
//
...
// ─────────────────────────────────────────────────────────────────────────────
//
// 1. `+` Builds a Single FlowInstructionChain
//    ─────────────────────────────────────
//    • MapValue + DistinctUntilChanged + Filter + Tap + MapValue is ONE
//      FlowInstruction. Each `+` returns another FlowInstruction, and the
//      whole expression is a _FlowInstructionChain under the hood.
//
// 2. Operators Stay Stock, Policy Stays Pure
//    ────────────────────────────────────────
//    • riskOf is a plain top-level function: no await, no I/O, no money.
//    • MapValue wraps the pure policy; DistinctUntilChanged dedupes on
//      the Decision; Filter keeps the actionable outcomes; Tap runs the
//      book side effect; the last MapValue re-exposes Decision.
//
// 3. One toHandle, One Live Cell
//    ─────────────────────────────
//    • The chain is a blueprint until `toHandle(source: attemptIn.cell)`
//      compiles it into `authHandle.cell`.
//
// 4. The Trade-Off: Stock DistinctUntilChanged Has No Reset
//    ───────────────────────────────────────────────────────
//    • ACK appends its row but cannot reach inside the stock operator.
//      The scenario order (approve between repeats) keeps the visible
//      output identical. If you need true ACK reset, hand-roll a tiny
//      resettable FlowInstruction link and chain it in place of
//      DistinctUntilChanged.
//
// 5. TestCell on Ingress (Security Boundary)
//    ──────────────────────────────────────
...
// 7. Two Locks
//    ───────────
//    • The Receptor lock covers the chained operators (riskOf → Distinct
//      → Filter → Tap book writes).
...
/// book write), but the shape supports any [Decision].
...
/// The payload carried through the operator chain.
///
/// The first MapValue projects an [AuthAttempt] into this record so the
/// later operators can dedupe on the [decision] while the Tap side effect
/// still has the original [attempt] to write the books.
...
Decision riskOf(AuthAttempt a, TissueSet<String> block) {
...
///   `ackIn`, plus the one chained gate (`cardAuth` / `authCell`).
...
class CardAuthHarness {
  ...
  ///
  /// The chained gate subscribes to this cell via
  /// `toHandle(source: attemptIn.cell)`.
  late final IngressHandle<AuthAttempt> attemptIn;

  /// The ACK ingress (3DS / analyst id).
  ///
  /// An ACK appends an `ACK` row to the ledger. It never rebuilds the
  /// graph and, with stock operators, cannot reset the chain's
  /// DistinctUntilChanged.
  late final IngressHandle<String> ackIn;

  // ---------------------------------------------------------------------------
  // Flow gate — ONE FlowInstructionChain, ONE handle.
  // ---------------------------------------------------------------------------

  /// The single card-auth chain (MapValue → DistinctUntilChanged →
  /// Filter → Tap → MapValue).
  late final FlowInstruction<Cell, Pulse, Pulse> cardAuth;

  /// The one `toHandle` produced by [installGate].
  FlowHandle<Pulse<dynamic>>? authHandle;

  /// The gate cell — emits a [Decision] when the chain lets it through.
  late final Cell authCell;
  ...
  /// Number of DECLINE pulses that made it past DistinctUntilChanged +
  /// Filter.
  int declines = 0;

  /// Number of STEP-UP pulses that made it past DistinctUntilChanged +
  /// Filter.
  int stepUps = 0;
  ...
  // ---------------------------------------------------------------------------
  // Issuer pump working list + failure injection.
  // ---------------------------------------------------------------------------
  ...
  /// Builds every Cell and Tissue, wires the gate, and installs the
  /// observers.
  ///
  ...
  /// 3. Build the one `FlowInstructionChain` and materialize it with
  ///    `toHandle` exactly once.
  /// 4. Attach the ACK observer.
  Future<void> install() async {
    ...
    // --- ACK observer -------------------------------------------------------
    //
    // On an ACK pulse, this observer appends an ACK row to the ledger.
    //
    // With stock operators, ACK cannot reset DistinctUntilChanged (no
    // public reset API). The scenario sequence is designed so the visible
    // output is unchanged: every repeated DECLINE is separated by an
    // approve, which already moves the stock distinct latch.
    _observers.add(Cell.observe(
      ...
      effect: (Pulse pulse) {
        ...
        print('[ledger] ACK $authId — ack received '
            '(stock distinct latch untouched)');
      },
    ));
  }

  // ---------------------------------------------------------------------------
  // installGate — ONE FlowInstructionChain built from stock Rx operators
  // ---------------------------------------------------------------------------

  /// Builds the chained [cardAuth] gate and materializes it once.
  ///
  /// ### Execution Order is Critical!
  /// Runs exactly once from [install]. ACK does NOT call this again.
  ///
  /// ### The chain (left to right)
  /// 1. `MapValue<AuthAttempt, CardAuthRecord>` — run `riskOf` and carry
  ///    both the attempt and its decision (Rx `map`).
  /// 2. `DistinctUntilChanged<CardAuthRecord>` — drop consecutive equal
  ///    decisions, keyed only on `decision` (Rx `distinctUntilChanged`).
  /// 3. `Filter<CardAuthRecord>` — keep only `decline` / `stepUp`
  ///    (Rx `filter`).
  /// 4. `Tap<CardAuthRecord>` — write the books as a side effect without
  ///    changing the payload (Rx `tap` / `do`).
  /// 5. `MapValue<CardAuthRecord, Decision>` — project the record back
  ///    down to the public Decision output (Rx `map`).
  ///
  ...
  // ---------------------------------------------------------------------------
  // Book writes (the Tap side effect).
  // ---------------------------------------------------------------------------

  /// Ledger + issuer enqueue for a distinct DECLINE.
  void _onDecline(AuthAttempt a) {
    declines++;
    ledger.add(LedgerEntry(
      kind: 'DECLINE',
      authId: a.authId,
    ...
    final job = IssuerJob(authId: a.authId, decision: Decision.decline);
    issuerQ.addLast(job);
  ...
  /// Ledger row for a distinct STEP-UP. No money, no issuer job.
  void _onStepUp(AuthAttempt a) {
    stepUps++;
    ledger.add(LedgerEntry(
      kind: 'STEP-UP',
      authId: a.authId,
...
Future<void> main() async {
  print(
      '========================================================================');
  print(' card-auth-pipeline(Cell)-Demo.dart');
  print(' One FlowInstructionChain:');
  print(' MapValue → DistinctUntilChanged → Filter → Tap → MapValue');
  print(' Flow decides. Tissue keeps the books.');
  print(
      '========================================================================');
  ...
  // Seed — 7200 / 5411 / present / vel 0
  //
  // Demonstrates: the bus + the chain. `riskOf` returns `approve`, which
  // DistinctUntilChanged records and Filter drops. The ledger stays empty
  // (initial Tissue population is silent).
  // -------------------------------------------------------------------------
  ...
  // 1 — repeat 7200 / 5411
  //
  // Demonstrates: DistinctUntilChanged on `approve`. The same decision
  // repeats, so the chain drops it before Filter/Tap.
  // -------------------------------------------------------------------------
  ...
  //
  // Demonstrates: TissueSet feeds `riskOf`; the Tap side effect writes
  // the DECLINE audit trail. The blocklist is mutated at runtime; the
  // next CNP attempt with MCC 7995 is declined.
  // -------------------------------------------------------------------------
  ...
  // 3 — grocery+present then 7995 CNP again
  //
  // Demonstrates: DistinctUntilChanged `approve` → `decline`. The first
  // attempt is approve (moves the stock distinct latch), the second is
  // decline (passes the chain).
  // -------------------------------------------------------------------------
  ...
  // 4 — 7995 again (should not re-decline)
  //
  // Demonstrates: DistinctUntilChanged holds. The previous decision is
  // still `decline`, so the duplicate is suppressed.
  // -------------------------------------------------------------------------
  ...
  // 5 — 200000 on 7995
  //
  // Demonstrates: DistinctUntilChanged keys on Decision, not amount.
  ...
  // STEP-UP — 60000 CNP 5411
  //
  // Demonstrates: the second Decision outcome flows through the same
  // chain. The blocklist does not apply to 5411; amount + CNP triggers
  // STEP-UP.
  // -------------------------------------------------------------------------
  ...
  // 6 — 7200 / 5411 present then ACK
  //
  // Demonstrates: ACK appends its row. With stock operators it cannot
  // reset DistinctUntilChanged; the following approve (this attempt)
  // already moves the latch. No money moves yet.
  // -------------------------------------------------------------------------
  ...
  // 7 — 7995 CNP 7200
  //
  // Demonstrates: after the approve in scenario 6, the stock distinct
  // latch moved off `decline`, so the same decline passes again.
  // -------------------------------------------------------------------------
  ...
  // 9 — ACK, recover, 7995, issuer fail-once
  //
  // Demonstrates: the queue + retry inside the Tap side effect. The first
  // issuer attempt fails; the pump retries and succeeds. The DECLINE
  // count remains 1 for this step.
  // -------------------------------------------------------------------------
  ...
  // Trailer
  //
  // Confirms the money invariant, the read-only deputy's shared storage,
  // and the chain's outcome counters.
  // -------------------------------------------------------------------------
  print('');
  print(
      '------------------------------------------------------------------------');
