# Walkthrough requirement — card auth pipeline (Cell)

**Demo:** `card-auth-pipeline(Cell)-Demo.dart` (executable; this file is its requirement)  
**Variant of:** `card-auth-pipeline(tissue)-WalkThrough.md`  
**Siblings:**  
- `card-auth-pipeline(tissue)-WalkThrough.md` — two gates + observers  
...
This variant composes `riskOf` + Distinct + DECLINE / STEP-UP book
writes into **one** `FlowInstructionChain` built from five stock Rx
operators — `MapValue → DistinctUntilChanged → Filter → Tap → MapValue`
— materialized with **one** `toHandle`. ACK and money stay outside that
chain.  
...
5. [Parts](#parts)
   ...
   - [Deputies](#deputies)
   - [Operator chain](#operator-chain-one-flowinstructionchain)
   - [Receptor](#receptor)
...
- DECLINE vs STEP-UP as two outcomes of one pipeline
- Distinct and ACK on the same materialized handle
- issuer retry logic
...
presentIn                       ─┘
                                 ...
                                 attemptIn          Flow
                                      │
                                      ▼
                        ONE FlowInstructionChain (stock operators)
                          MapValue:  AuthAttempt → (attempt, decision)
                          DistinctUntilChanged: drop repeat decision
                          Filter:   keep decline / stepUp
                          Tap:      ledger + issuerQ book writes
                          MapValue: (attempt, decision) → Decision
                                      │
                                      ▼
                              authHandle.cell      (one toHandle)
                                      │
                                      ├─ Tap → _onDecline
                                      │     ledger.add(...)              TissueList
                                      │     issuerQ.addLast(job)         TissueQueue
                                      │     _driveIssuer (retry once)
                                      └─ Tap → _onStepUp
                                            ledger.add(...)              TissueList

ackIn  3DS / analyst ── ACK observer
                         ledger.add(ACK ...)              TissueList
                         (stock distinct latch is NOT reset)
...
| DECLINE vs STEP-UP | one chain, two `Decision` arms in `Filter` / `Tap` |
| No duplicate DECLINE | `DistinctUntilChanged` before `Filter` |
| Issuer I/O | `TissueQueue` (audit) + `_driveIssuer` retry from `Tap` |
...
Do **not** `ledger.add` inside `riskOf` or inside a `MapValue` link.
`riskOf` returns a `Decision`; the `Tap` side effect writes Tissue. If
you mutate Tissue inside a map link, you hide the lock and you cannot
unit-test the policy with a bare `AuthAttempt`.

Do **not** replace Distinct with “ledger already has this attempt.”
The ledger is history. Distinct is the **current alarm latch**.
...
The chain also defines a record payload so later links can see both the
attempt and its decision:

```dart
typedef CardAuthRecord = ({AuthAttempt attempt, Decision decision});
```

Suggested `riskOf(AuthAttempt a, TissueSet<String> block) → Decision`:
...
| else | `approve` (Filter drops it; Distinct still records it) |
...
| `authCell` | `toHandle` output cell | emits `Pulse<Decision>` when the chain lets it through |
...
|---|---|---|---|---|
| `ledger` | `TissueList<LedgerEntry>` | append-only: allow `add` / `addAll`; deny `remove`, `clear`, `[]=` | Tap side effect, ACK observer, hold/capture/void | compliance deputy |
...
| `holdsMap` | `TissueMap<String, Hold>` | value cents `> 0`; key non-empty | ACK-approve path, capture, void | settlement |
| `mccBlock` | `TissueSet<String>` | 4-digit MCC string | ops scenario | `riskOf` |
| `issuerQ` | `TissueQueue<IssuerJob>` | `capacity: 32` | Tap side effect (DECLINE) | issuer pump |
...
### Operator chain (one FlowInstructionChain)

| Link | Type | Job |
|---|---|---|
| Snapshot | `MapValue<AuthAttempt, CardAuthRecord>` | `(a) => (attempt: a, decision: riskOf(a, mccBlock))` |
| Dedupe | `DistinctUntilChanged<CardAuthRecord>` | `equals` compares only `.decision` |
| Actionable | `Filter<CardAuthRecord>` | keeps `decline` / `stepUp` |
| Books | `Tap<CardAuthRecord>` | switches to `_onDecline` / `_onStepUp` |
| Emit | `MapValue<CardAuthRecord, Decision>` | `(r) => r.decision` |

The `+` operator composes the five links into one `FlowInstructionChain`:
...
Order is mandatory: `map → distinct → filter → tap → map`.

Library `DistinctUntilChanged` is a stock operator with no public reset
API. That is the documented trade-off of this variant: ACK appends its
row but does not reset the latch. The scenario sequence (approve
between repeated declines) keeps the visible output identical.

### Receptor

`cardAuth.toHandle(source: attemptIn.cell)` once at `installGate()`.
The returned handle is stored as `authHandle`; `authHandle.cell` is the
live gate cell (`authCell`). One lock on the gate cell.

Tissue has its **own** lock on each collection. Do not assume the
Receptor lock covers `ledger.add`. The `Tap` side effect and the money
methods still take the tissue locks one write at a time: two locks, two
owners, two talk-track sentences.

### Operators the demo must actually call

Flow: `MapValue`, `DistinctUntilChanged`, `Filter`, `Tap`,
`FlowInstruction.operator +` (the chain), `toHandle`, `Cell.observe`,
`Cell.ingress(testRule:)`.

Tissue: `TissueList` / `TissueList()`, `TissueValue`, `TissueMap`,
`TissueMapNucleus`, `TissueSet`, `TissueQueue`, `TestTissue(...)`,
`.unmodifiable`, `Cell.observe` on the tissue (tissues **are** Cells).
...
Dart method, `placeHold`, called by the harness owner. If the
`available` TestTissue rejects (would go negative), **do not** leave a
map row behind — remove the map entry and do not append HOLD.
...
try/compensate in `placeHold` and say so in the file header.
...
| `AuthAttempt` / `Decision` / `LedgerEntry` / `Hold` / `IssuerJob` / `CardAuthRecord` | domain |
...
| `installGate` | the `+` chain: `MapValue → DistinctUntilChanged → Filter → Tap → MapValue` |
| `_onDecline` / `_onStepUp` / `_driveIssuer` | Tap side effect + issuer pump |
| `cardAuth.toHandle(source: attemptIn.cell)` | materialize the chain once |
| `placeHold` / `capture` / `voidHold` | TissueValue + TissueMap |
| `main` | seed + scenarios 1–13 |

`installGate` runs **once**. ACK only appends an `ACK` row and then
calls `placeHold` when the talk track says “approve after 3DS.”
...
|---|---|---|---|
| Seed | 7200 / 5411 / present / vel 0 | one attempt, **no** DECLINE, ledger still empty | bus + chain riskOf; initial Tissue is silent |
| 1 | repeat 7200 / 5411 | no DECLINE | DistinctUntilChanged on `approve` |
...
| 3 | grocery+present then 7995 CNP again | **DECLINE** after recover | Distinct `approve`→`decline` |
| 4–5 | 7995 again, then 200000 on 7995 | **no** new DECLINE | Distinct holds; ledger does **not** grow |
| STEP-UP | 60000 CNP 5411 | **STEP-UP** + ledger row | second Decision arm; blocklist does not apply to 5411 |
| 6 | 7200 / 5411 present then ACK | ACK row; stock distinct latch untouched | ACK ≠ new `toHandle` |
| 7 | 7995 CNP 7200 | **one** DECLINE | approve moved the stock distinct latch |
| 8 | amount `-1`, MCC `"99"` | TestCell lines; **no** attempt; ledger unchanged | Cell ingress ≠ Tissue |
| 9 | ACK, recover, 7995, issuer fail-once | **one** DECLINE; `issuerAttempts` includes retry | queue + `_driveIssuer` retry |
...
These are the steps `card-auth-pipeline(Cell)-Demo.dart` actually
runs. Numbers match the `── N ──` banners in the console. Seed is
unnumbered but required: without it the stock distinct operator has no
first `approve`, and “repeat does not decline” in step 1 is
meaningless.

**Seam reminder at every step.** Flow answers “may this attempt
become a DECLINE or STEP-UP pulse?” Tissue answers “what did the
books just record, and did money move?” The `Tap` side effect and the
ACK observer are the only glue. `riskOf` never writes `available`.
`placeHold` never runs inside the chain.
...
- ACK does **not** reset the stock `DistinctUntilChanged` (no public
  reset API); the ACK row now says so. The scenario order keeps the
  visible output identical.
...
**Lesson:** snapshot bus + chain riskOf. Initial Tissue population is
silent.
...
- The chain runs: `MapValue` → `(SEED, approve)`;
  `DistinctUntilChanged` passes the first value; `Filter` drops
  `approve`; `Tap` never runs.
...
**Lesson:** DistinctUntilChanged on `approve`. The chain records the
decision but no book write fires for `approve`.
...
- `DistinctUntilChanged` sees the same consecutive decision and drops
  the pulse before `Filter` / `Tap`.
...
**Lesson:** TissueSet feeds `riskOf`. The `Tap` side effect is the
audit trail. Two locks: gate cell then TissueList / TissueQueue.
...
2. `publishAttempt('2')` — CNP + blocked MCC → `riskOf` =
   `decline`.
3. `DistinctUntilChanged` sees `approve` → `decline` and passes.
   `Filter` keeps `decline`. `Tap` calls `_onDecline`.
4. `_onDecline`: `ledger.add(DECLINE 2)`,
...
- `3a` grocery present → `approve`. `DistinctUntilChanged` moves off
  `decline` to `approve`; `Filter` drops it (no book write).
- `3b` 7995 CNP → `decline`. Distinct `approve` → `decline` passes;
  `Tap` calls `_onDecline`, which appends DECLINE 3b + ISSUER 3b.
...
**Lesson:** DistinctUntilChanged holds `decline`. Same consecutive
decision, dropped before Filter.
...
**Lesson:** DistinctUntilChanged keys on **Decision**, not amount.
Raising cents does not create a new decline while the previous
decision is still `decline`.
...
7995 + CNP is still `decline` first in `riskOf`.

---

### STEP-UP — 60000 CNP 5411

**Lesson:** second Decision outcome through the same chain.
Blocklist does not apply to 5411.
...
- `riskOf`: 5411 not in `mccBlock`; amount ≥ 50_000 and CNP →
  `stepUp`.
- `DistinctUntilChanged` sees `decline` → `stepUp` and passes.
  `Filter` keeps `stepUp`. `Tap` calls `_onStepUp`.
- `_onStepUp` appends `STEP-UP SU-1`. No issuer job (this demo
  only enqueues on DECLINE).
...
**Lesson:** ACK appends its row; the stock distinct latch is **not**
reset. No money yet.
...
- Attempt 6 is `approve` — `DistinctUntilChanged` moves off
  `stepUp` to `approve`; `Filter` drops it.
- `ackIn` observer appends `ACK 6`. It does **not** touch the
  chain's distinct state.

**Must print**

```text
── 6 ── 7200 / 5411 present then ACK
[ledger] ACK 6 — ack received (stock distinct latch untouched)
...
**Lesson:** after the intervening approve, the same decline passes the
stock distinct operator again.
...
- ACK 9-pre appends its row (no latch reset).
- `9a` grocery present → approve (the stock distinct latch moves to
  approve).
...
[ledger] ACK 9-pre — ack received (stock distinct latch untouched)
...
- `riskOf` → `stepUp` (60000 CNP 5411). `Tap` → `_onStepUp`
  appends `STEP-UP H-1`.
- ACK appends `ACK H-1`.
...
[ledger] ACK H-1 — ack received (stock distinct latch untouched)
...
setMcc('7995'); setPresent(cardNotPresent); setAmount(7200)
  ...
  publishAttempt → attemptIn
    authCell Receptor path: the FlowInstructionChain
      MapValue → (attempt 2, decline)
      DistinctUntilChanged: approve → decline, pass
      Filter: decline, pass
      Tap → _onDecline
        ledger.add(...)           → ElementAdded<LedgerEntry>
        issuerQ.addLast(...)      → ElementAdded<IssuerJob>
        _driveIssuer              → ledger.add(ISSUER ...)
      MapValue → Decision.decline
```

Scenario 8 stops at `amountIn.emit` / `mccIn.emit`. Tissue is idle.

Scenario 10 never enters `riskOf` for the debit. The caller calls
`placeHold` after ACK. If you debit inside `riskOf`, two STEP-UPs
before ACK will steal the balance twice.
...
| Event | Lock |
|---|---|
| Chain links (`MapValue → DistinctUntilChanged → Filter → Tap → MapValue`) | gate cell (`authCell`) Receptor path |
| `_onDecline` / `_onStepUp` (called from `Tap`) | each Tissue lock, taken one write at a time |
| `ledger.add` | TissueList lock |
| `available` / `held` write | that TissueValue lock |
| `holdsMap[id] = ...` | TissueMap lock |
| issuer pump (`_driveIssuer`) | plain Dart Future — no cell lock |

Do not “fix” a race by putting `ledger.add` inside `riskOf`.
Teach the two locks.
...
1. `dart run card-auth-pipeline(Cell)-Demo.dart` prints every row
   in the scenario table with the **Result** column matched.
2. Grep shows exactly **one** `toHandle(` call for the risk gate,
   inside `installGate`, none inside ACK.
...
6. Scenario 9 shows a retry in `issuerAttempts` and still
   **one** DECLINE pulse.
...
10. No Dart `List<LedgerEntry>` is the system of record. The
    `_issuerWork` list is a pump queue, not a ledger.
...
12. The gate is **one** `FlowInstructionChain` composed with `+` from
    stock operators: `MapValue → DistinctUntilChanged → Filter → Tap →
    MapValue`.
13. The `Tap` link is the only chain link that writes Tissue; ACK
    only appends its row and never calls `toHandle`.
...
| Artifact | Name |
|---|---|
| This requirement / walkthrough | `card-auth-pipeline(Cell)-WalkThrough.md` |
| Working demo | `card-auth-pipeline(Cell)-Demo.dart` |
| Architecture | `card-auth-pipeline(Cell)-ARCHITECTURE.md` |
| Features | `card-auth-pipeline(Cell)-FEATURES.md` |
| Two-gate sibling | `card-auth-pipeline(tissue)-WalkThrough.md` |
| Flow-only sibling | `card-auth-pipeline(enhanced)-WalkThrough.md` |

Same pairing style as `ICU-alarm-pipeline(enhanced)-*`.
This file is the **one-chain** lesson. Keep the tissue two-gate files.
