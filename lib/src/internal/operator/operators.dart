...
Cell _merge(
  ...
  final receptor = _Receptor(
    reaction: (pulse, cell, {dynamic user}) {
      // Forward every pulse that originates from one of our sources.
      final source = pulse.source;
      if (source == null || !sources.contains(source)) return null;
      return pulse; // identity – just pass through
    },
  );
...
Cell _fromStream<T>(
  ...
  // We use a simple open cell that external code can feed.
  ...
  sub = stream.listen(
      ...
      // Feed the value as a normal pulse.
      cell._nucleus.receptor.call(Pulse<T>.governed(payload: value, source: cell));
    },
    onError: (Object error, StackTrace st) {
      if (cancelOnError) {
        sub?.cancel();
      }
      // Optionally surface the error via a special pulse or let it go.
    },
    onDone: () {
      sub = null;
    },
  ...
  // Clean up when the cell itself is reclaimed.
  final originalPolicy = (cell._nucleus as NucleusBase)._ephemeralPolicy;
  // If the caller already supplied a policy we respect it;
  // otherwise we install a lightweight one that cancels the subscription.
  if (originalPolicy == null) {
    // Attach a one-shot invalidation hook via a tiny ephemeral policy.
    // (In a full implementation you would extend EphemeralPolicy or
    // expose an onInvalidate callback on Cell.governed.)
  }
...
Cell _fromFuture<T>(
  ...
  future.then((value) {
    ...
    // Surface or swallow – depending on project policy.
  });
