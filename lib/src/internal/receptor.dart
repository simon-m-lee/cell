...
class _Receptor<C extends Cell> extends ReceptorBase<C>{
  ...
  _Receptor({super.instruction, super.preProcess, super.postProcess, super.reaction, super.user, super.isGoverned}) : super();
...
abstract class ReceptorBase<C extends Cell> implements Receptor<C> {
  ...
  /// The transformed pulse, or `null` if the signal was terminated.
  @override
  FutureOr<Pulse?> call(covariant PulseBase pulse) {

    if (pulse is Shell) {
      return pulse.scrutinize(this, null);
    }
...
class ReceptorAsync<C extends Cell> implements Async {
  ...
  /// 3. **Queueing**: Places the pulse into a FIFO execution queue. The
  ///    returned [Future] completes once the pulse is safely queued.
  /// 4. **Sequential Processing**: A background worker drains the queue
  ///    one-by-one, ensuring atomic execution within the cell's perimeter.
  ///
  ...
  /// - **Fire-and-Forget**: By default, `await call(...)` does not wait for
  ///   the pulse to finish propagation. It only waits for the queue entry.
  ...
  ///
  /// ### Parameters:
  /// - [pulse]: The data signal to be processed.
  ...
  ///
  /// ### Returns:
  /// A [Future] that completes once the pulse has been successfully accepted
  ...
  Future<void> call(PulseBase pulse, {bool serializedCompletion = true, void Function({Pulse? result, required Pulse input})? hook}) async {

    if (pulse is Shell) {
      return pulse.scrutinize(_receptor, null, {#serializedCompletion: serializedCompletion});
    }
    ...
    if (!_unawaitedLock.canLock) {
      return;
    }

    unawaited(_unawaitedLock.synchronized(() async {
      PulseBase? out;
